...
当前首页已实现对话、参考演示、候选方案校验及设计交接。真实模型代理已实现，见 [模型配置与部署](MODEL_CONFIGURATION.md)。选择通用/航发模型后，通过当前网站的同源后端调用以下接口；浏览器使用登录会话自动鉴权。参考演示只有显式载入图19预设，没有自然语言求解能力。

## 请求

`POST /api/design-assistant/chat`，`Content-Type: application/json`。前端使用 HttpOnly 会话Cookie，并为写请求附带 X-CSRF-Token。提供商 API Key 仅在配置弹窗中提交给后端，不写入浏览器存储。部署方在相同源配置服务端代理，密钥从服务器私有数据库或初始化环境读取；可以映射通用与航发模型到不同提供商。不要让浏览器直接访问带密钥的模型 URL。
...
实际请求 parameterContract 自动包含 `pritchard.js` 中全部11字段，上例仅展示一项格式。provider 为 `general` 或 `domain`。messages 是当前会话用户与助手纯文本，最多60条、单条用户6000字符。用户草稿不会发送；参考按钮发送显式“载入参考方案”。当前不使用流式协议，前端135秒超时并支持 AbortSignal 取消；停止仅取消前端等待，当前后端不保证供应商取消计算或计费。
...
- 401：登录过期或未登录；403：来源或CSRF校验失败；404/405/501/503：接口未接入或暂不可用；其他非2xx：请求失败。
...
当前桥接服务已提供上述配置和对话路由，采用平台认证、并发限额、兼容协议适配和业务 JSON 校验。后续可继续扩展其他供应商协议。系统提示要求先补足边界条件，再调用平均线工具，逐字段保留来源，严禁将给定效率目标当成预测值。域模型与通用模型共用此业务协议，提供商消息/工具调用格式由后端转换。
