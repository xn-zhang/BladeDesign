...
网页现已支持 **初始设计生成参数包 → CFD批次仿真 → AI按批次构建数据集**，包括JSON导入、限流提交、暂停/取消和选中失败项重试。优先使用 [批量数据生产指南](BATCH_PRODUCTION.md) 中的新流程，它复用平台登录会话。

下方是保留的旧版脚本途径：为每份设计构建请求，逐个调用 API，服务器按计算槽位并行运行。脚本使用独立计算服务令牌，其本地文件名去重机制不具备新版生产批次的追溯与恢复能力。
