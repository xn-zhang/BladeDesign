...
export async function openAdmin(){
  ...
  if(!dialog){
    ...
    dialog.querySelector('[data-copy]').onclick=async()=>{try{await navigator.clipboard.writeText(dialog.querySelector('[data-code] input').value);message('已复制，请单独发给测试者。');}catch{message('请选中邀请码手动复制。');}};
...
async function load(){
  ...
  try{
    ...
    for(const i of invites.invites){const row=el('div');row.className='account-row';row.append(el('span',`${i.id.slice(0,8)} · ${i.used_by?'已使用':i.revoked?'已撤销':i.expires_at*1000<Date.now()?'已过期':'可使用'}`));if(!i.used_by&&!i.revoked&&i.expires_at*1000>Date.now()){const b=el('button','撤销');b.onclick=()=>act(b,'invites/'+i.id+'/revoke',{});row.append(b);}group.append(row);}
