...
export function initPersonalWorkspace({getDesign,applyDesign,getConditions,applyConditions,assistant,openDesign}){
  ...
  document.body.append(dialog);const $=s=>dialog.querySelector(s);let kind='designs',serial=0;
  ...
  async function operate(action,item,selectedKind){
    ...
    try{
      if(action==='load'){
        if(!confirm(`载入“${item.name}”将替换当前${selectedKind==='designs'?'设计与工况':'对话草稿'}，是否继续？`))return;
        const data=await api(selectedKind+'/'+item.id);checkIdentity(identity);if(id!==serial)return;
        ...
        const name=prompt('新的名称',item.name);if(name===null)return;await api(selectedKind+'/'+item.id+'/rename',{name});if(id===serial)await load();
      }else if(confirm(`删除“${item.name}”？`)){await api(selectedKind+'/'+item.id+'/delete',{});if(id===serial)await load();}
  ...
  document.addEventListener('aeroblade:sessionchange',()=>{serial++;if(dialog.open)dialog.close();$('#personal-name').value='';$('#personal-notes').value='';$('[data-list]').replaceChildren();message('');});
